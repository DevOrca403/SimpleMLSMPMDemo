 ...
 */
void get_neighbor_base(Vector2f* _pos_begin,
                       float _cell_space,
                       Vector2i* _base_dest,
                       size_t _len) {
    float inv_cell_space = 1.0f / _cell_space;
    for(int loop = 0; loop < _len; ++loop, ++_pos_begin, ++_base_dest) {
        Vector2i base_coord = ((*_pos_begin) * inv_cell_space - Vector2f::Constant(0.5f)).cast<int>();
 ...
 * @param _len Length of inputs (i.e. number of particles)
 */
void get_neighbor_fracture(Vector2f* _pos_begin,
                           Vector2i* _base_begin,
                           float _cell_space,
                           Vector2f* _fracture_dest,
                           size_t _len) {
    float inv_cell_space = 1.0f / _cell_space;
    for (int loop = 0; loop < _len; ++loop, ++_pos_begin, ++_base_begin, ++_fracture_dest) {
        Vector2f fracture_coord = ((*_pos_begin)) * inv_cell_space - (*_base_begin).cast<float>();
 ...
 * @param _len Length of inputs (i.e. number of particles)
 */
void get_weight(Vector2f* _fracture_begin,
                std::array<Vector2f, 3>* _weight_dest,
                size_t _len) {
    for (int loop = 0; loop < _len; ++loop, ++_fracture_begin, ++_weight_dest) {
        (*_weight_dest) = {
                0.5f * (Vector2f::Constant(1.5f) - (*_fracture_begin)).array().square().matrix(),
                Vector2f::Constant(0.75f) - ((*_fracture_begin) - Vector2f::Constant(1.0f)).array().square().matrix(),
                0.5f * ((*_fracture_begin) - Vector2f::Constant(0.5)).array().square().matrix()
        };
    }
}
